...
class SELayer(nn.Module):
    def __init__(self, inp, oup, reduction=4):
        ...
        self.fc = nn.Sequential(
                nn.Conv2d(inp, _make_divisible(inp // reduction), 1, 1, 0,),
                nn.ReLU(),
...
class PPLCNet(nn.Module):
    def __init__(self, scale=1.0, num_classes=1000, dropout_prob=0.2):
        ...
        block = DepSepConv
        for k, in_c, out_c, s, use_se in self.cfgs:
            input_channel = _make_divisible(in_c * scale)
            output_channel = _make_divisible(out_c * scale)
        ...
        self.avgpool = nn.AdaptiveAvgPool2d((1, 40))
