...
from torch.nn import AdaptiveAvgPool2d, BatchNorm2d, Conv2d, Dropout, Hardsigmoid, Hardswish, Identity, Linear, ReLU
...
class LearnableAffineBlock(nn.Module):
    def __init__(self, scale_value=1.0, bias_value=0.0, lr_mult=1.0,
        ...
        self.scale = torch.nn.Parameter(torch.tensor(scale_value), requires_grad=True)
        self.bias = torch.nn.Parameter(torch.tensor(bias_value), requires_grad=True)

        # nn.init.constant_(self.scale, scale_value)
...
class SELayer(nn.Module):
    def __init__(self, channel, reduction=4, lr_mult=0.1):
        super(SELayer, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Sequential(
                nn.Conv2d(channel, make_divisible(channel // reduction), 1, 1, 0,),
                nn.ReLU(),
                nn.Conv2d(make_divisible(channel // reduction), channel, 1, 1, 0),
                Hardsigmoid()
        )
