...
class ParallelSARDecoder(BaseDecoder):
    ...
    def _2d_attention(self,
        ...
        attn_weight = attn_weight.permute(bsz, T, -1)
        ...
        if self.pred_concat:
            ...
            holistic_feat = torch.Tensor.expand(
    ...
    def forward_test(self, feat, out_enc, img_metas):
        ...
        start_token = torch.full(
            (bsz, ), fill_value=self.start_idx, dtype=torch.int64)
        # bsz
        start_token = self.embedding(start_token)
        # bsz * emb_dim
        emb_dim = start_token.shape[1]
        start_token = start_token.unsqueeze(1)
        # start_token = torch.Tensor.expand(start_token, shape=[bsz, seq_len, emb_dim])
...
if __name__=="__main__":
    sar_head = SARHead(192, 232)
    dummy = torch.rand([30,3,192])
    output = sar_head(dummy)